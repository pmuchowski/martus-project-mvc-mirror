...
import org.martus.client.swingui.jfx.setupwizard.tasks.BulletinExportEncryptedZippedMbaTask;
...
public class BulletinsListController extends AbstractFxLandingContentController implements FolderSelectionListener
{
	public BulletinsListController(UiMainWindow mainWindowToUse, BulletinListProvider bulletinListProviderToUse, FxCaseManagementController caseManagementControllerToUse)
	{
		...
		caseManagementController = caseManagementControllerToUse;
	...
	public boolean isAllBeingDisplayed()
	{
		return bulletinTableProvider.getAllFolderBeingDisplayedBooleanProperty().get();
	}
	...
	private void exportBulletins(UniversalId[] bulletinsIdsToExport)throws Exception
	{
		...
		if(showModalYesNoDialog("Export", "export", EnglishCommonStrings.CANCEL, exportController))
		{
			...
			if (exportController.shouldExportEncryptedInsideZip())
				doExportEncryptedZippedBulletinMbas(bulletinsIdsToExport, exportFile);
			else if(exportController.shouldExportEncrypted())
	...
	private void doExportEncryptedZippedBulletinMbas(UniversalId[] bulletinIdsToExport, File exportFile)
	{
		AbstractExportTask task = new BulletinExportEncryptedZippedMbaTask(getMainWindow(), bulletinIdsToExport, exportFile);
		doExport(task, "ExportZippedBulletins");
	}
