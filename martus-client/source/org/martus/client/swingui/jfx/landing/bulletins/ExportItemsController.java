...
import org.martus.client.swingui.filefilters.ZipFileFilter;
...
public class ExportItemsController extends FxController
{
	...
	public void initialize(URL location, ResourceBundle bundle)
	{
		...
		zipForUploading.selectedProperty().addListener(new UploadableZipStatusChangeHandler());
		
		updateControls(shouldExportEncrypted());
	}

	protected void enableEncryptExportFileCheckBox()
	{
	...
	public boolean shouldExportEncryptedInsideZip()
	{
		return isZipCheckBoxSelected();
	}
	
	protected class UploadableZipStatusChangeHandler implements ChangeListener<Boolean>
	{
		public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) 
		{
			zipCheckboxChanged(newValue);
		}
	}
	...
	protected void zipCheckboxChanged(Boolean newValue)
	{
		if (newValue)
		{
			enableEncryptExportFileCheckBox();
		...
		encryptExportFile.setDisable(newValue);
		updateExportFilename();
	}
	...
	private String getExportFilenameBasedOnEncryptionStatus(String fileNameOnly)
	{
		...
		if (isZipCheckBoxSelected())
			fullNameWithCorrectFileExtension += ZipFileFilter.ZIP_FILE_EXTENSION;
		
		else if(shouldExportEncrypted())
	...
	private String getExportFileExtension()
	{
		...
		if (isZipCheckBoxSelected())
			return ZipFileFilter.ZIP_FILE_EXTENSION;
	...
	private FormatFilter getFormatFilter()
	{
		...
		if (isZipCheckBoxSelected())
			return new ZipFileFilter(localization);
	...
	private boolean isZipCheckBoxSelected() 
	{
		return zipForUploading.isSelected();
	}
	...
	@FXML
	private CheckBox zipForUploading;
