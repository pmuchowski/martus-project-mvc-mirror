...
public class SpecTableModel extends AbstractTableModel
{
	...
	public void addUniqueSpecs(FieldSpec[] specsToAdd)
	{
		if(specsToAdd == null)
			return;
		
		specs.removeAll(Arrays.asList(specsToAdd));
		specs.addAll(Arrays.asList(specsToAdd)); 
		fireTableDataChanged();
	}
	
	public void removeFieldSpecs(FieldSpec[] fieldSpecToRemove)
	{
		specs.removeAll(Arrays.asList(fieldSpecToRemove));
		fireTableDataChanged();
	}
