...
public class MartusGridField extends MartusField
{
	...
	public MartusField getSubField(String tag, MiniLocalization localization)
	{
		...
		try
		{
			...
			for(int i = 0; i < gridSpec.getColumnCount(); ++i)
			{
				...
				String thisLabel = sanitizeLabel(thisColumnSpec.getLabel());
				//FIXME MEDIUM: this is a patch fix to get reports working for a particular use.
				//Once the build is finished, undo this change. 
				thisLabel = thisLabel.replaceAll("\n", " ");
				thisLabel = quotedEscape(thisLabel);				
	...
	//FIXME MEDIUM this method is part of a report bug patch and will be removed.  Its also a duplicate. 
	private static String quotedEscape(String raw)
	{
		String result = raw;
