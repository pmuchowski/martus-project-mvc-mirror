...
public class BulletinHeaderPacket extends Packet
{
	...
	void setAllowNonAuthorUpload(String data)
	{
		if (data.equals(ALLOW_NON_AUTHOR_UPLOAD))
			enableNonAuthorUpload();
		else
			disableNonAuthorUpload();
	}
	...
	public boolean isAuthorizedToUpload(String publicKey)
	{
		...
		if(!allHQsCanProxyUpload)
			return false;
		boolean hqAuthorizedToRead = isHQAuthorizedToRead(publicKey);
		if (hqAuthorizedToRead)
			return true;
		
		if (allowNonAuthorUpload())
			return true;
		
		return false;
	}

	public boolean allowNonAuthorUpload()
	{
		return allowNonAuthorUpload;
	}
	
	public void enableNonAuthorUpload()
	{
		setNonAuthorUpload(true);
	}
	
	public void disableNonAuthorUpload()
	{
		setNonAuthorUpload(false);
	}
	
	private void setNonAuthorUpload(boolean shouldAllowNonAuhtorUpload)
	{
		allowNonAuthorUpload = shouldAllowNonAuhtorUpload;
	}
	...
	protected void internalWriteXml(XmlWriterFilter dest) throws IOException
	{
		...
		if (allowNonAuthorUpload())
			writeElement(dest, MartusXml.AllowNonAuthorUpload, ALLOW_NON_AUTHOR_UPLOAD);
		else
			writeElement(dest, MartusXml.AllowNonAuthorUpload, PROHIBIT_NON_AUTHOR_UPLOAD);
	...
	private final static String ALLOW_NON_AUTHOR_UPLOAD = "1";
	private final static String PROHIBIT_NON_AUTHOR_UPLOAD = "0"; 
	...
	private boolean allowNonAuthorUpload;
}
