...
abstract public class AbstractMockMartusServer extends MartusServer
{
	public AbstractMockMartusServer(TestCaseEnhanced testCase) throws Exception
	{
		this(new TempDirectory(), new MockServerDatabase(), testCase);
	}
	
	public AbstractMockMartusServer(Database databaseToUse, TestCaseEnhanced testCase) throws Exception
	{
		this(new TempDirectory(), databaseToUse, testCase);
	}
	
	public AbstractMockMartusServer(File dataDir, TestCaseEnhanced testCase) throws Exception
	{
		this(dataDir, new MockServerDatabase(), testCase);
	}
	
	public AbstractMockMartusServer(File dataDir, Database databaseToUse, TestCaseEnhanced testCase) throws Exception
	{
		super(dataDir, new LoggerToNull(), ServerMetaDatabaseForTesting.getEmptyDatabase(testCase));
		
		getStore().setSignatureGenerator(createServer());
	...
	{
		return new String(this.serverForClients.requestUploadRights(authorAccountId, tryMagicWord));
	}
	...
	public String uploadBulletinChunk(String authorAccountId, String bulletinLocalId, int totalSize,
			int chunkOffset, int chunkSize, String data, String signature)
	{
		lastClientId = authorAccountId;
		lastUploadedBulletinId = bulletinLocalId;				
	...
	public synchronized void subtractMaxFailedUploadAttemptsFromServerCounter()
	{
		...
		this.subtractMaxFailedUploadRequestsForIp(clientIp);
	}
	...
	protected static class TempDirectory extends File
	{
	...
	abstract protected AbstractMockMartusSecurity createServer() throws Exception;
